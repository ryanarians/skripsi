...
class C_Apriori extends Controller
{
    ...
    public function setupPerhitunganApriori()
    {
        ...
        return view('main.apriori.setup', $dr);
    }
    ...
    public function prosesAnalisaApriori(Request $request)
    {
        ...
        $dateStart = $request -> timestart;
        $dateEnd = $request -> timeend;
        // 'support': support,
        ...
        $totalProduk = M_Penjualan::whereDate('tanggal', '>=', $dateStart)->whereDate('tanggal', '<=', $dateEnd)->distinct('no_faktur')->count('no_faktur');
        $minSupp = $totalProduk;
        $minConfidence = $totalProduk;
        ...
        $dataProduk = M_Penjualan::whereDate('tanggal', '>=', $dateStart)->whereDate('tanggal', '<=', $dateEnd)->distinct()->get('kd_barang');
        foreach($dataProduk as $produk){
            $kdProduk = $produk -> kd_barang;
            $totalTransaksi = M_Penjualan::whereDate('tanggal', '>=', $dateStart)->whereDate('tanggal', '<=', $dateEnd)->where('kd_barang', $kdProduk) ->count();
            ...
            $supp -> date_start = $dateStart;
            $supp -> date_end = $dateEnd;
        ...
        foreach($qProdukA as $qProdA){
            ...
            foreach($qProdukB as $qProdB){
                ...
                $jumB = M_Nilai_Kombinasi::where('kd_pengujian', $kdPengujian)->where('kd_barang_a', $kdProdukB) -> count();
                if($jumB > 0){
                    ...
                    if($kdProdukA == $kdProdukB){
                        ...
                        $nk -> total_semua_transaksi = $totalProduk;
    ...
    {   
        ...
        // $totalProduk = M_Penjualan::whereBetween('created_at', [$date -> dateStart, $date -> dateEnd])->count();
        ...
        $dr = [
            ...
            'support' => $dataPengujian->min_supp
        ];
